...
func Compile(ctx context.Context, req *rpc.CompileRequest, outStream, errStream io.Writer, debug bool) (r *rpc.CompileResponse, e *status.Status) {
	...
	if req.GetShowProperties() {
		return r, status.New(codes.Unknown, builder.RunParseHardwareAndDumpBuildProperties(builderCtx).Error())
	} else if req.GetPreprocess() {
		return r, status.New(codes.Unknown, builder.RunPreprocess(builderCtx).Error())
	}

	// if it's a regular build, go on...
	if err := builder.RunBuilder(builderCtx); err != nil {
		return r, status.New(codes.Unknown, err.Error())
	}
	...
	for _, lib := range builderCtx.ImportedLibraries {
		...
		if err != nil {
			return r, status.Newf(codes.PermissionDenied, "converting library %s to rpc struct: %w", lib.Name, err)
		}
